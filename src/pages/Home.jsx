...
const internships = [
  ...
  {
    id: 'xunfei', company: '科大讯飞', role: '中国移动 · 跨模态数据检索产品经理',
    period: '2025.11—2026.01', color: '#FF8C00', tag: 'NLP · 多模态 · B端AI',
    points: [
      '【业务目标】聚焦中国移动智能客服业务数据利用率低、检索效率低、关联精度低三大痛点，构建跨模态数据检索体系，解决"查重""找相关"需求',
      '【需求转化】收集运营商痛点问题 2300+，结合人工标注与平台清洗分析，以生产管线—平台功能—RICE 为分析基准，发现高优先级检索优化方向',
      '【竞品调研】调研亚马逊云 SageMaker Data Wrangler、火山引擎、阿里云的可视化查询、多模态检索、向量索引功能及交互逻辑',
      '【数据建构】基于 NLP、CLIP、ASR 等特征向量化原理，协调业务确定标注关键词规范，制定人工标注交叉审核机制和抽样检查',
      '【解决方案】从关键词检索升级为跨模态检索—向量化存储—知识图谱应用—多路召回（关键词+向量）重排序，全程跟进原型绘制、评审、验收',
      '【成效指标】V1.12.0 上线与九天训推平台、梧桐大数据贯通；关键词检索准确率 95%，向量检索召回率 98%；A/B 测试原准确率 82%，跨模态重排后 TOP-10 准确率 94.7%',
    ],
  },
  {
    ...
    points: [
      '【业务目标】小度教育名著阅读用户没兴趣、不坚持、不会用，根据教育性与趣味性需求，制定剧情驱动 AI 生成绘本与交互的解决方案',
      '【问题发现】基于教育平板与小度在家端用户行为埋点数据，发现 3–9 岁儿童名著阅读功能留存时长 <8 分钟、留存率 <35%',
      '【竞品调研】调研 AI 绘本（ReadKidz、StoryBird.ai、childbook.ai、豆包等）及绘本学习（凯叔、Kada），对比用户分层、生成内容形式与互动方式差异',
      '【互动功能】以教学为核心目标设计"剧情拆解—绘本生成—多模态输出—动态校准"内容生产流程，约束灯光、动作、配音与剧情创作互动',
      '【Prompt 工程】基于角色设定与主线剧情约束，设计结构化 Prompt，涵盖剧集拆解、场景描述、剧情交互，持续调优引导模型生成',
      '【技术对接】融合文心一言与 Stable Diffusion，参与制定角色一致性校验、知识点难度分级、年龄认知适配、多模态内容生产等 7 大算法模块需求',
      '【上线效果】SDK 接入小度教育儿童平板，灰度发布后迭代动态交互与奖励机制，覆盖用户 10 万+；A/B 测试儿童知识答题准确率均值提升 47%，家长 NPS 均值 8.1 分',
    ],
  },
]
